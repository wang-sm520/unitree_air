#author: Xing Rui
#train: CUDA_VISIBLE_DEVICES=3 ./unitree_rl_lab.sh -t --task Unitree-DT114-XR-Stand --resume --load_run 2026-03-20_14-38-46 --checkpoint model_60700.pt
...
# Import the NEW robot configuration
...
class EventCfg:
    ...
    # UPDATED: torso_Link -> waist_yaw_link
...
class CommandsCfg:
    ...
    base_velocity = mdp.UniformLevelVelocityCommandCfg(
        ...
        ranges=mdp.UniformLevelVelocityCommandCfg.Ranges(
            lin_vel_x=(-0.3, 0.8), lin_vel_y=(-0.3, 0.3), ang_vel_z=(-0.3, 0.3)
        ),
...
class RewardsCfg:
    ...
    base_angular_velocity = RewTerm(func=mdp.ang_vel_xy_l2, weight=-1.0)
    joint_vel = RewTerm(func=mdp.joint_vel_l2, weight=-0.001)
    joint_acc = RewTerm(func=mdp.joint_acc_l2, weight=-0.0000001)
    action_l2 = RewTerm(func=mdp.action_l2, weight=-0.05)
    action_rate = RewTerm(func=mdp.action_rate_l2, weight=-0.001)
    dof_pos_limits = RewTerm(func=mdp.joint_pos_limits, weight=-1.0)#惩罚关节角度超出软件限位
    energy = RewTerm(func=mdp.energy, weight=-0.000002)
    ...
    # NOTE: DT114 target height is 0.88m based on leg kinematics
    base_height = RewTerm(func=mdp.base_height_l2, weight=-1, params={"target_height": 0.88})
    # -- feet
    feet_air_time_cfg = RewTerm(
        func=mdp.feet_air_time_positive_biped,
        weight=1.0,
        params={
            "command_name": "base_velocity",
            "threshold": 0.5,   # 单次支撑时间奖励上限（秒），可根据期望步频调整
...
class TerminationsCfg:
    ...
    joint_vel_out_of_bounds = DoneTerm(func=mdp.joint_vel_out_of_limit,params={"asset_cfg": SceneEntityCfg("robot")})
    joint_pos_out_of_bounds = DoneTerm(func=mdp.joint_pos_out_of_limit,params={"asset_cfg": SceneEntityCfg("robot")})
...
@configclass
class DT114StandEnvCfg(ManagerBasedRLEnvCfg):
...
@configclass
class DT114StandPlayEnvCfg(DT114StandEnvCfg):
    def __post_init__(self):
        super().__post_init__()
        self.scene.num_envs = 32
        self.scene.terrain.terrain_generator.num_rows = 2
        self.scene.terrain.terrain_generator.num_cols = 10
        self.commands.base_velocity.resampling_time_range = (1.0, 1.0)
        self.commands.base_velocity.rel_standing_envs = 0.0
        self.commands.base_velocity.ranges = mdp.UniformLevelVelocityCommandCfg.Ranges(
            lin_vel_x=(-0.3, 0.6), lin_vel_y=(0.0, 0.0), ang_vel_z=(0.0, 0.0)
        )
