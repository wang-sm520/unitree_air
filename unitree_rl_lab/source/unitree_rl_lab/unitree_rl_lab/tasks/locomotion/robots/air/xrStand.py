...
class RobotSceneCfg(InteractiveSceneCfg):
    ...
    # sensors
    # UPDATED: torso_Link -> pelvis_link (merged due to fixed joint)
...
class EventCfg:
    ...
    # UPDATED: torso_Link -> pelvis_link
...
class CommandsCfg:
    ...
    base_velocity = mdp.UniformLevelVelocityCommandCfg(
        ...
        ranges=mdp.UniformLevelVelocityCommandCfg.Ranges(
            lin_vel_x=(-0.3, 0.6), lin_vel_y=(-0.2, 0.2), ang_vel_z=(-0.2, 0.2)
        ),
        limit_ranges=mdp.UniformLevelVelocityCommandCfg.Ranges(
            lin_vel_x=(-0.5, 0.8), lin_vel_y=(-0.3, 0.3), ang_vel_z=(-0.3, 0.3)
        ),
    )
...
class RewardsCfg:
    ...
    feet_air_time = RewTerm(
        ...
        weight=1.0,                    # start with 0.5–2.0; higher = stronger lift
        params={
            "sensor_cfg": SceneEntityCfg(
                "contact_forces",
                body_names=["left_ankle_roll_link", "right_ankle_roll_link"],
            ),
            "mode_time": 0.1,         # minimum swing time before reward kicks in (tune)
            "velocity_threshold": 0.01, # ignore very slow movement
        },
    )'''
    gait = RewTerm(
        func=mdp.feet_gait,
...
class DT114StandPlayEnvCfg(DT114StandEnvCfg):
    def __post_init__(self):
        ...
        self.commands.base_velocity.ranges = mdp.UniformLevelVelocityCommandCfg.Ranges(
            lin_vel_x=(-0.3, 0.4), lin_vel_y=(0.0, 0.0), ang_vel_z=(0.0, 0.0)
        )
